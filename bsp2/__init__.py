...
def scale_sparse_minmax(input_exp_mat: csr_matrix) -> csr_matrix:
...
def binary_distance_matrix_threshold(
...
def spvars(input_csr_mat: csr_matrix, axis: int) -> np.ndarray:
...
def test_scores(
    ...
    input_exp_mat_norm = scale_sparse_minmax(input_exp_mat_raw).transpose()
    ...
    def var_local_means(d_val: float) -> np.ndarray:
        patches_cells = binary_distance_matrix_threshold(input_sp_mat, d_val)
    ...
    var_x = np.column_stack([var_local_means(d_val).A.ravel() for d_val in (d1, d2)])  # type: ignore
    var_x_0_add = spvars(input_exp_mat_raw, axis=1).A.ravel()  # type: ignore
...
def granp(
    ...
    t_matrix_sum = test_scores(input_sp_mat, input_exp_mat_raw, d1, d2)
